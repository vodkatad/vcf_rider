...
fn decode_genotype(geno: String, phased: &mut bool) -> (bool, bool) {
        ...
        loop {
            if let Ok(_) = self.reader.read(&mut record) {
                ...
                let genotypes = (0..self.reader.header.sample_count() as usize).map(|s| {
                                    ...
                                    decode_genotype(geno_str, & mut phased)
                                }).collect_vec(); //useful to pre alloc?  Vec::<(bool, bool)>::with_capacity(self.samples.len());
                // cannot break inside of a closure therefore we continue here if there were some samples not phased.
                // inefficient because it will decode all genotypes 
                if ! phased {
                    continue;
                }
                let pos = Coordinate { chr: chr, start: coord, end: coord+1}; // Right now only Snps.
                return Some(Mutation { id: id, pos: pos, sequence_ref: refe, sequence_alt: alte, genotypes : genotypes})
            } else {
                return None;
            }
        }
