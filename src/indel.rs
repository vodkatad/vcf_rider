    ...
    impl IndelRider {
        pub fn new(snps_buffer: & VecDeque<mutations::Mutation>, n_overlapping: usize, n_samples: usize) -> IndelRider {
        ...
        ///
        pub fn count_groups(snps_buffer: & VecDeque<mutations::Mutation>, n_overlapping: usize, groups: &mut Vec<usize>, n_samples: usize) {
        ...
        pub fn get_group_info(&self, window: & mut mutations::Coordinate, next_pos: & mut u64, snps_buffer: & mut VecDeque<mutations::Mutation>, n_overlapping: usize, info: & mut Vec<(usize, MutationClass)>) {
            ...
            for (i_snp, snp) in snps_buffer.iter_mut().enumerate() {
                if i_snp < n_overlapping { // i >= n_overlapping we have finished the overlapping snps (the last one is just waiting in the buffer)
                    ...
                    for i_sample in this_g_v.to_vec() {
                        let index = i_sample % self.n_samples_tot;
